...
#sub: 최초로 만난 해당값만 바꿔줌 / gsub: 모든 해당값을 바꿔줌
...
# 이것을 cbind로 풀면 두번째, 다섯번째 변수는 오류가 난다.
# 둘이 변수 개수가 다를 때는 left_join은 왼쪽 데이터셋(test1)을 기준으로 정렬,
# right_join은 오른쪽(test2)기준,
# inner_join은 일치하지 않는 값 무조건 제외
# full_join은 모두 포함
...
# 결측치 포함된 상태로 분석
mean(df$score)  # 평균 산출 ---> 결과는 NA
sum(df$score)   # 합계 산출 ---> 결과는 NA
# 결측치 있는 행 제거하기
...
df_nomiss  
# 결측치가 하나라도 있으면 제거하기, 결측치 있는 행 제거
df_nomiss2 <- na.omit(df)  # 모든 변수에 결측치 없는 데이터 추출

#분석에 필요한 데이터까지 손실 될 가능성 유의
# 함수의 결측치 제외 기능 이용하기 - na.rm = T
mean(df$score, na.rm = T)  # 결측치 제외하고 평균 산출. 행전체를 삭제하는 게 아니라 특정 NA'값'만!
...
boxplot(mpg$hwy)  # 결측치는 동그라미로 나타남

# 인터넷에서 복사하고 싶은 소스 복사금지 돼있을 때?
# 인터넷 - 설정 - 고급 - 사이트설정 - 자바스크립트 해제
...
# 날짜, 시간
(today <- Sys.Date()) # 밖의 소괄호는 대입도 하고 화면에도 내보내기 위함
format(today, "%Y년 %m월 %d일") # %m은 월을 숫자로 출력
format(today, "%d일 %B %Y년") #%B는 월 이름으로 출력. 영문이었으면 september가 나왔을 것.
format(today, "%y") #연도를 두 자리로 나타냄 e.g. 19
format(today, "%Y") #연도를 네 자리로 나타냄 e.g. 2019
format(today, "%B")
format(today, "%b") #월 축약형 e.g. 11
format(today, "%a") #요일 축약형 e.g. 화
format(today, "%A") #요일 e.g. 화요일
weekdays(today) # 요일 e.g. 화요일
months(today) 
quarters(today)
today
...
as.Date('22119',format='%d%b%y') 

x1 <- "2019-01-10 13:30:41"
# 문자열을 날짜형으로
as.Date(x1, "%Y-%m-%d %H:%M:%S") #m은 월, M은 분
# 문자열을 날짜+시간형으로
...
as.Date("2018/01/01 08:00:00") - as.Date("2018/01/01 05:00:00") #순서 다르게 하려면 format문자 줘야한다
as.POSIXct("2018/01/01 08:00:00") - as.POSIXct("2018/01/01 05:00:00")
#POSIXct는 저장할 때 지금까지의 초시간이 보관됨
as.POSIXlt("2018/01/01 08:00:00") - as.POSIXlt("2018/01/01 05:00:00")
#POSIXlt는 리스트 객체로 날짜와 시간정보 처리
# 날짜만 상관있다면 Sys.Date
# 시간까지 해야되면 POSIXct, POSIXlt
...
str(ct) 
str(lt) 
unclass(ct) # 초시간이 numeric으로 나온다
# unclass; 객체에 대한 정보를 풀어서 보여준다.
unclass(lt) # List가 쪼개져 나온다.
# 11월이 아닌 10월로 나오는 이유는? $mon은 0부터 시작된다.
# 항상 추출할 때 +1 해줘야 한다.
# year도 1900을 더해줘야 한다.
lt$mon+1
lt$hour
...
as.POSIXlt("2019/08/22")$wday # '/'나 '-'나 상관없다   #wday=2는 화요일
as.POSIXlt("2019/08/23")$wday
as.POSIXlt("2019/08/24")$wday
as.POSIXlt("2019/08/25")$wday
as.POSIXlt("2019/11/03")$wday #R은 일요일이 0



# 데이터 시각화
# 고수준함수: 주인공. 하나의 플롯에는 하나의 주인공. 여러개 한꺼번에 그릴 수 없음.
# plot; 산전도
# boxplot: 연속형, 수치형자료의 분포 파악. 박스그래프
# hist: 연속형 데이터 값의 분포를 파악. 히스토그램
# 저수준함수: 보완하는 역할. 고수준함수가 그려진 상태에서 덧그리는 역할.
# legend; 범례. 데이터 파악할 때 많이 씀. 보고서 쓸 때는 ggplot2 많이 사용
rainbow(10)

# pch

국어<- c(4,7,6,8,5,5,9,10,4,10)  # 데이터셋 하나만 주면 y축이 되고, x축은 인덱스.
plot(국어)

plot(국어, type="o", col="blue")  # type은 값을 이어주는 점, 라인의 타입     
